...
**No configuration surface edited `input` when this shipped.** It joined `compat`, `reasoningEfforts`, `thinkingBudgets`, and `headers` as a settings-document field, and the model-list editor stayed a hand-written form over id, name, and the two capacities. The Models settings page now edits it: [the modality-editing decision](../feature/2026-08-31-models-page-input-modalities.md) reversed this exclusion on the surface alone and left every resolution rule below unchanged. The row patch spreads the stored row before applying changes, and adoption keeps an existing row over a rediscovered candidate, so a hand-written `input` still survives both.
