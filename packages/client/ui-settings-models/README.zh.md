...
键入的 API 密钥按其自身字段判定：去除首尾空白后必须非空，且每个字符都必须是可打印 ASCII（`[\x21-\x7E]`），这正是 HTTP 头值能够携带的字符集——与 `@deepseek-ai/dsh-llm` 中的 `normalizeApiKey` 互为镜像，此处复刻是因为源平面拆分禁止导入它。与粘贴的 `NAME=value` 环境行一致或包裹在匹配引号内的值，会作为同样的格式失败被拒绝。空 id、重复 id、空显式名称以及不可读、非正数或小数的容量都会在任何写入之前失败。某行的输入模态在下列情况会失败：列表含有 `text` 和 `image` 之外的模态、出现重复、为空——这些只有手工编辑 `settings.yaml` 才会带到页面，因为 checkbox 组写不出它们。DeepSeek 的 `models` 是一个按值整体替换的数组：编辑器先显示继承的有效行，直到第一次模型编辑把完整数组物化进用户层，重置则取消该覆盖。

### 输入模态

两个 family 的行都在行展开区内渲染同一组 checkbox，但字段名不同：`llm-deepseek` 读 `inputModalities`，`llm-pi-ai` 读 `input`。名称不同是因为空值语义不同——deepseek 的 schema 把该字段默认为 `['text']`，而 pi-ai 把缺失的 `input` 读作"此处没有答案"，并通过已安装 catalog 条目、进而 route 的 `defaultInput` 来解析。共用同一个名称需要在每次读取时做映射，且 pi-ai 那一半会丢掉"声明为纯文本"与"继承"之间的区别。

因此 `src/client/modality.ts` 按 family 写入各自的字段。text 是下限，不可清除：清除最后一个模态会恢复它，而不是写入空列表——pi-ai 会把空列表读作"什么都不接受的模型"，deepseek 的 schema 也会拒绝它。显式声明不会因为看起来像默认值而被丢弃——在 pi-ai 侧那样做会把用户刚刚收窄的行又放大回其 catalog 条目所说的样子。
